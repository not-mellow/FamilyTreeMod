    ...
    class NewActions : MonoBehaviour
    {
        ...
        public static void action_family(WorldTile pTile = null, string pDropID = null)
        {
            ...
            for (int i = 0; i < MapBox.instance.temp_map_objects.Count; i++)
            {
                ...
                if (!pActor.stats.unit || pActor.kingdom == null || FamilyOverviewWindow.familyActors.Contains(pActor) || pActor.kingdom.id == "nomads_human" || pActor.kingdom.id == "nomads_elf" || pActor.kingdom.id == "nomads_orc" || pActor.kingdom.id == "nomads_dwarf")
                {
                    continue;
                }
        ...
        public static void loadSave()
        {
            ...
            foreach(SavedActor actor in loadedHeadsData)
            {
                ...
                Actor actorComp = newObject.GetComponent<Actor>();
                ...
                foreach(SavedActor childActor in actor.savedChildren)
                {
                    ...
                    Actor childActorComp = newChildObject.GetComponent<Actor>();
                    childActorCompList.Add(childActorComp);
                }
                FamilyOverviewWindow.loadParent(actorComp, childActorCompList, actor.savedHead);
            }
            ...
            foreach(SavedActor sActor in loadedActorsData)
            {
                ...
                FamilyOverviewWindow.loadParent(actorComp, childActorCompList, null);
            }
