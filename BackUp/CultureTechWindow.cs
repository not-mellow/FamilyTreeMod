...
namespace FamilyTreeMod
{
    class CultureTechWindow : MonoBehaviour
    {
        ...
        private static void initUICulture()
        {
            ...
            {
                GameObject bgHolder = NewActions.addCanvasRenderer(WindowManager.windowContents["cultureWindow"], createPos(0, y));
                RectTransform bgRect = bgHolder.GetComponent<RectTransform>();
                bgRect.sizeDelta -= new Vector2(10, 50);
                Button bgButton = bgHolder.AddComponent<Button>();
                bgButton.onClick.AddListener(() => initUITech(culture));
                NewActions.addText(culture.name, bgHolder, 20);
                y++;
            }
            return;
        }
        ...
        private static void initUITech(Culture culture)
        {
            ...
            GameObject textGo = NewActions.addText(culture.name, WindowManager.windowContents["cultureWindow"], 15).gameObject;
        ...
        private static Vector2 createPos(int rowIndex, int colIndex)
        {
            float startX = 130;
            float startY = -50;
            Vector2 size = new Vector2(40, 50);
